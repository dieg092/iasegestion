      ...
      Token.deleteOne(
        ...
        if (!err) {
          ...
          const mailOptions={
            ...
            html: '<div><p>' + user[0].name + ' ' + user[0].lastName + ' (' + user[0].email + ')' + ' quiere acceder a iasegestion.com.</p><p>Para ello:</p><p>1. Verificar PDF Firmado Digitalmente: '+ keys.urlBucket + user[0].digitalSignature + '</p><p>2. Activar su cuenta: ' + keys.path + 'admin/usuarios/' + user[0]._id + '</p><p>3.Enviar claves de acceso.</p><p></p><p>*Si el PDF no es correcto. Hay que eliminar el PDF en la ficha del cliente para que pueda volver repetir el proceso.</p></div>',
          };
